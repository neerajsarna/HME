...
void legendre_compute_glr0 ( int n, double *p1, double *p2 );
void legendre_compute_glr1 ( int n, double *roots, double *ders );
void legendre_compute_glr2 ( double p1, int n, double *roots, double *ders );
...
*/  
  ...
  {
    legendre_compute_glr2 ( p1, n, &x[n/2], &w[n/2] );
  }
/*
...
*/
{
  ...
  for ( k = 0; k < n; k++ )
  {
    dk = ( double ) k;
    *p1 = - dk * pm2 / ( dk + 1.0 );
    *p2 = ( ( 2.0 * dk + 1.0 ) * pm1 - dk * ppm2 ) / ( dk + 1.0 );
    pm2 = pm1;
    pm1 = *p1;
    ppm2 = ppm1;
    ppm1 = *p2;
...
*/
  for ( j = 0; j < n; j++ )
  {
    for ( i = 0; i < m; i++ )
    {
      fprintf ( output, " %0.15f", table[i+j*m] );
    }
    fprintf ( output, "\n" );
  }
/*
